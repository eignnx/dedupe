...
class DedupeProgram:
    ...
    def review_dupes(self):
        ...
        # Display duplicates.
        print("Main Directory\t\t\tBackup Directory\t\t\tFilesize (bytes)")
        for main, backup in self.kill_list:
            print(os.path.basename(main["Path"]),
                  os.path.basename(backup["Path"]),
                  main["Bytes"], sep="\t\t\t")
            ...
            if inp == "D":
                ...
                if inp == "YES":
                    ...
                    for main_file, _ in self.kill_list:
                        ...
                        print(f"DELETING: {main_file['Path']}")
                        os.remove(main_file["Path"])

                    print()
                    print(f"{bytes_removed} bytes have been deleted from the MAIN FILE SYSTEM")
